...
class SpeedController():
	...
	def _release_break(self):
		if abs(self.current_speed - abs(self.expected_speed)) > 1.0:
			threading.Timer(1, self._release_break).start()
			return
		else:
			self._set_brake_intensity(0.0)

	def slow_to(self, speed, brake_intensity):
		self.stop = 1
		self.set_expected_speed(speed)
		self._set_brake_intensity(brake_intensity)
		threading.Timer(1, self._release_break).start()
